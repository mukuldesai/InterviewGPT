...
"[project]/src/app/mockdata/jobData.ts [app-rsc] (ecmascript)": ((__turbopack_context__) => {
"use strict";
...
function filterJobs(params) {
    ...
    if (params.query) {
        ...
        filteredJobs = filteredJobs.filter((job)=>job.job_title.toLowerCase().includes(queryLower) || job.job_description.toLowerCase().includes(queryLower) || job.employer_name.toLowerCase().includes(queryLower));
    }
    // Filter by location
    if (params.location) {
    ...
    if (params.experience) {
        ...
        filteredJobs = filteredJobs.filter((job)=>{
            ...
            if (expLower.includes('entry') || expLower.includes('junior')) {
                return title.includes('junior') || title.includes('entry') || description.includes('entry') || description.includes('junior') || description.includes('0-2 years') || description.includes('1-3 years') || description.includes('entry-level');
            }
            if (expLower.includes('mid')) {
                return title.includes('mid') || description.includes('mid') || description.includes('3-5 years') || description.includes('2-4 years');
            }
            if (expLower.includes('senior')) {
                return title.toLowerCase().includes('senior') || title.toLowerCase().includes('sr.') || description.includes('senior') || description.includes('5+ years') || description.includes('7+ years');
            }
            return true;
        });
    }
...
//# sourceMappingURL=src_app_mockdata_jobData_ts_ab2d7a41._.js.map